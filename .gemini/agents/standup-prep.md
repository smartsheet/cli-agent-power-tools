...
*   **Workflow:**
    1.  **Ask User:** "I've prepared the standup brief. Want me to draft a Slack update from this via `status-comms-writer`?"
...
*   **Workflow:**
    1.  **Immediately Execute Tool Call:** `print(default_api.invoke_agent(agent_name='status-comms-writer', prompt='This is the final step in an automated chain. Please draft a Slack update based on the following standup brief, then call complete_task(). Data: [JSON_OUTPUT]'))`
    2.  **Note:** Replace `[JSON_OUTPUT]` with the JSON data for the brief you generated.
