...
Also call `get_sheet_summary` once without any filters to capture the sheet's total row count from metadata. This is used in the coverage line to show total rows alongside rows evaluated. If the unfiltered call returns `is_sampled: true`, record the total from the metadata — do not use the sampled row list as the total.
...
Call `find_in_sheet` with:
- `filter` = `=[Owner]@row = ""` — substituting the exact owner column name from Step 1
...
1. Ask the user: "I've completed the risk scan and found [X] items. Want me to hand this to `standup-prep` to build your pre-meeting brief?"
2. On confirmation: execute `invoke_agent`, passing your `risk_items` array in the prompt.

**2. If in Automated Chain Mode (your prompt explicitly says "This is an automated chain"):**
1. Immediately execute: `print(default_api.invoke_agent(agent_name='standup-prep', prompt='This is an automated chain. Use these risk scanner findings to generate a standup brief and then proceed to your next step without confirmation. Data: [JSON_OUTPUT]'))`
2. Replace `[JSON_OUTPUT]` with the `risk_items` array from your scan results.
