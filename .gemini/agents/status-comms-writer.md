...
**Either:**
- Upstream output from any Power Tool (JSON payload passed directly in automated pipelines, or natural language summary from the orchestrating Gemini instance in conversational use — both forms work)
...
1. **Accept any upstream input.** In conversational use, the orchestrating model reads upstream Power Tool output and passes context in natural language — no raw JSON parsing required. In automated pipelines, the upstream JSON is passed directly as a string argument. Both forms work.
